...
I considered Both precision and recall to be important in this analysis. If acustomer expects a flight to be on time and it’s delayed, the customer is frustrated. So recall is important. But if the customer expects the flight to be delayed, leaves later for the airport, and misses the (actually on-time) flight, the customer is still frustrated. So precision is important. So F1 is arguably the best metric.
